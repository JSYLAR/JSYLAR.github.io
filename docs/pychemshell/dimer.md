...
prmtop = file.parse('F6H_box.prmtop', fmt='amber_prmtop')
...
my_pdb = Fragment(coords='F6H_box.pdb',charges=charges)
...
# Define QM method
my_qm =ORCA(functional="PBE0", basis="def2-SVP",d4=True,convergence="TightSCF",nprocs='24',charge='-2', mult='2', broken='2,1',scftype='uhf',auxbasis='def2/J',exchange="RIJCOSX")
# Define the MM method (DL_POLY)
my_mm = DL_POLY(ff='F6H_box.prmtop', rcut=99.99)
...
my_opt = Opt(theory=my_qmmm, frag2=my_mol_2, dimer=True, algorithm="lbfgs", trust_radius="const")
my_opt.run()
```
